...
import scrapy
import re
from jd_spider.items import JdElectItem
...
class JddqSpider(scrapy.Spider):
    ...
    def parse(self, response):
        # 电脑办公，手机及配件，数码
        li_list = response.xpath("//div[@class='col'][1]/div[@class='category-item m'][6]//dl[position()<7]/dd/a[position()<5]|//div[@class='col'][1]/div[@class='category-item m'][2]//dl[1]/dd/a[1]|//div[@class='col'][1]/div[@class='category-item m'][2]//dl[3]/dd/a|//div[@class='col'][1]/div[@class='category-item m'][4]//dl[1]/dd/a[position()<8]")
        for li in li_list:
    ...
    def get_detail_info(self,response):
        ...
        try:
            ...
            cat = re.findall(r'cat: \[(.*?)\],', response.body.decode('gb18030', 'ignore'))[0]
            skuId = re.findall(r'skuid: (\d+),', response.body.decode('gb18030', 'ignore'))[0]
    ...
    def get_activity_info(self,response):
        ...
        # 下拉详情的所有活动
        item['activity2'] = '，'.join(re.findall(r'"content":"(.*?)",', js_activity))\
            if len(re.findall(r'"content":"(.*?)",', js_activity)) > 0 else None
